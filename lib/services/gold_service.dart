...
class GoldService {
  ...
  Future<void> fetchGoldPrices() async {
    try {
      final futures = await Future.wait([
        _yahooService.fetchQuote('GC=F'), // Gold Futures
        _yahooService.fetchQuote('EGP=X'), // USD/EGP
      ]);
      ...
      if (goldQuote != null) {
        // Fix: Null safety
        final price = goldQuote.price ?? 0.0;
      ...
      if (egpQuote != null) {
        // Fix: Null safety
        final price = egpQuote.price ?? 0.0;
  ...
  double _calculatePrice(double purity) {
    ...
    // (Spot Price / 31.1035) * USD_EGP * Purity
